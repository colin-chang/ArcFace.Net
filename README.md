...
* 5.x 图像处理基`ImageSharp`开发，仅支持.Net 8+
* 自定义实现。用户使用任意图像处理库（如`
...
```bash
# 5.x ImageSharp
docker build -f ColinChang.ArcFace.ImageSharp/Dockerfiles/aspnet-8.0/Dockerfile -t colinchang/arcface5:aspnet-8.0 .
docker build -f ColinChang.ArcFace.ImageSharp/Dockerfiles/runtime-8.0/Dockerfile -t colinchang/arcface5:runtime-8.0 .
